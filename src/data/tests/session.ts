...
export const sessionTests: TestDefinition[] = [
  ...
  {
    id: 'SESS-013',
    vulnerabilityName: 'Overly Broad Session Cookie Scope',
    category: 'session',
    subcategory: 'Cookie Security',
    priority: 'Medium',
    description:
      'The session cookie is scoped to a parent domain or a broad path, so it is sent to every subdomain and application sharing that namespace. One weak or taken-over subdomain then yields the session for the whole estate.',
    testingGuidance: [
      'Inspect the Domain and Path attributes on session and authentication cookies: a leading-dot or parent-domain value shares the cookie estate-wide.',
      'List which hosts actually receive the cookie, and judge whether every one of them is in scope and equally trusted.',
      'Check for the __Host- prefix (or at least __Secure-) on session cookies, which pins them to the exact host.',
      'Chain with subdomain enumeration: a dangling or third-party-hosted subdomain plus a parent-scoped cookie is session theft without touching the application.',
    ],
    owasp: ['WSTG-SESS-02'],
    cwe: ['CWE-346'],
    applicability: rule.all(
      auth,
      rule.any(cookieSession, rule.includes('assetTypes', 'web-app')),
    ),
    aliases: [
      'Cookie Scope Too Broad',
      'Missing __Host- Cookie Prefix',
  ...
  },
  {
    ...
    cwe: ['CWE-347', 'CWE-345'],
    applicability: rule.all(auth, rule.includes('authMechanisms', 'jwt')),
